...
public class CachedEntityTCTest extends AbstractBaseTest {
    ...
    public void cachedItems() {
        final Statistics statistics = ((HibernateGenericDao) cachedEntityTCDao).
        ...
        cachedEntityTCDao.findByFieldOne(FIELD);
        long citCacheItAutoGenerated = statistics.getQueryCacheHitCount() - startCacheItCount;

        // Caches due to @CachteIt annotation on method
        cachedEntityTCDao.findAll();
        startCacheItCount = statistics.getQueryCacheHitCount();
        cachedEntityTCDao.findAll();
        long citCacheItOnClass = statistics.getQueryCacheHitCount() - startCacheItCount;

        // Caches due to NamedQuery hints
        cachedEntityTCDao.findCacheByFieldTwo(FIELD);
        startCacheItCount = statistics.getQueryCacheHitCount();
        cachedEntityTCDao.findCacheByFieldTwo(FIELD);
        long citCacheItNamedQuery = statistics.getQueryCacheHitCount() - startCacheItCount;

        // Senza la cache. Non viene messa in cache.
        cachedEntityTCDao.findNoCacheByFieldThree(FIELD);
        startCacheItCount = statistics.getQueryCacheHitCount();
        cachedEntityTCDao.findNoCacheByFieldThree(FIELD);
        long citNoCache = statistics.getQueryCacheHitCount() - startCacheItCount;
